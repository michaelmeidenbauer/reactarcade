export const copyBoard = (board) => {
  const boardCopy = board.map(row => {
    const cells = row.map(cell => ({
      isSegment: cell.isSegment,
      id: cell.id,
      isTreat: cell.isTreat
    }));
    return cells;
  }
  )
...
export const makeRandomPositionIndex = (board) => {
  ...
  while (randomCell.isSegment || randomCell.isTreat) {
...
export const getCoords = (targetClass, board) => {
  ...
  [...board].forEach((row, index) => {
    ...
    row.forEach((cell, cellIndex) => {
      if (cell.classString.includes(targetClass)) {
